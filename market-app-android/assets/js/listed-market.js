        ...
        ajaxHelper.get(getUrl('listing/list'), para, function (ret) {
            if (ret.success) {
                ...
                for (var i = 0; i < items.length; i++) {
                    ...
                    html += '<p class="date apyTime">申请日期：' + new Date(item.applyTime).Format("yyyy/MM/dd hh:mm:ss") + '</p>';
                    // html += '<p class="date useTime">生效日期：' + Util.formatDate(item.completeTime) + '</p>';
                    html += '<p class="date useTime">生效日期：' + new Date(item.completeTime).Format("yyyy/MM/dd hh:mm:ss") + '</p>';
                    html += '</div> </li>';
                }
                if (html != "") {
                    $('#ul').append(html);
