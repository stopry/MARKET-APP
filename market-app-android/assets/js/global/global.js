...
function backGame(){
    if(localStorage.getItem('isInstall')){//打开本地appZ家园
        var result = uexWidget.startApp(0,'org.cocos2d.Q7Game');
        if(!result){//如果失败就用浏览器打开
            var url = "http://www.zjiayuan.com";
            uexWidget.startApp("1", "android.intent.action.VIEW", '{"data":{"scheme":"'  + url + '"}}');
        }
    }else{//浏览器打开z家园游戏 网址
        var url = "http://www.zjiayuan.com";
        uexWidget.startApp("1", "android.intent.action.VIEW", '{"data":{"scheme":"'  + url + '"}}');
...
appcan.ready(function(){
    if(localStorage.getItem('isFromGame')){  
          initBackGameBtn();
     };
     
     appCanIsReady = true;
   
    // var sys = Util.checkEquipment();
      // // alert(sys);
      // if(sys=='ios'){
          // $("body").addClass('iosSys');
          // var bar = "<div class='iosStatusBar'></div>"
          // $("body").append(bar);
          // $(".header").addClass("iosHeader");
      // }
});
...
function showTips(msg,cla) {
    ...
    if(isIos()){
        $('#alertWeek').css('top','20px !important');
    }
    $alert.html(msg);
...
function showConDia(text,confirmFn,cancelFn){
    ...
    var confirDia =$(
        ...
        '<div class="conHeader"><i class="closeThis feedbtn">脳</i></div>'+
        '<div class="diaText">'+
        text+
        '</div>'+
        '<div class="btnWrap">'+
