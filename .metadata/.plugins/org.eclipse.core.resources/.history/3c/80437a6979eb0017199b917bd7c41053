...
var pid=localStorage.getItem("TRAN_PRO_ID");
...
function init(){
    ...
    totalPage = 10;
    isLoading = false;
    pid=localStorage.getItem("TRAN_PRO_ID");
    type=1;
    loadList1(1);
}
            ...
            if (ret.success) {
                ...
                for (var i = 0; i < items.length; i++) {
                    ...
                    var _url = '"html/entrust-order-detail.html?id='+item.id+'"'
                        ...
                        html += "<li class='rchRecItem AwardItem WrapB wx feedbtn "+(item.enType=='0'?'buy':'sale')+ "' onclick='openLocal("+_url+",!1,3)'>";

                    //html += "<li class=\"rchRecItem AwardItem WrapB wx "+(item.enType=="0"?"buy":"sale")+" feedbtn\" onclick=\"location='./entrust-order-detail.html?id="+item.id+"'\">";
        ...
        ajaxHelper.post(getUrl('tran/entrustRecordQuery'), para, function (ret) {
            if (ret.success) {
                ...
                for (var i = 0; i < items.length; i++) {
                    ...
                    //html += "<li class=\"rchRecItem AwardItem WrapB wx feedbtn "+(item.enType=="0"?"buy":"sale")+"\" onclick=\"location='./trade-record-detail.html?id="+item.id+"'\">";
                    html += '<div class="recLeft fl">';
                    html += '<div class="rchType userPic">';
