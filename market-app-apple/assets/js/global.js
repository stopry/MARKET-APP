...
//var apiHost = 'http://api.zjiayuan.com';
var apiHost = "http://192.168.19.89:8080";
var ajaxHelper = {
    //purl 是接口url
    //data是传入参数
    //callbacl是成功后的回调函数
           ...
           error:function(info){
               if (showLoad) {
                ...
                if (info.status == 401) {
                    ...
                    showTips('网络错误','error');
                    // showConDia("网络错误,请登录重试！",function(){
                        // openLocal('html/login.html');
                    // },function(){})
                }
                if(errcall){
...
function openPayPage(){
    function toOpen(){
        ajaxHelper.get(getUrl('applepay/check'),{version:appVersion},function(res){
            //否
           if(res.obj==0){
               var token = localStorage.getItem('AUTHORIZATION');
               token = encodeURI(token);
               location = 'http://www.senchen.vip/html/gold-recharge.html?token='+token;
           }
           //是
           if(res.obj==1){
               openLocal('html/gold-recharge.html',1);
           } 
        });
    };
    $.ajax({
    ...
    appcan.window.open({
        name:'pay',
        dataType:0,
        data:'http://www.senchen.vip/html/gold-recharge.html'
    });*/
}
